...
export const MessagesList = () => {
   const messages = useAITeacher((state) => state.messages);
   const playMessage = useAITeacher((state) => state.playMessage);
   const stopMessage = useAITeacher((state) => state.stopMessage); // Added this line
   const { currentMessage } = useAITeacher();
   const english = useAITeacher((state) => state.english);
   const classroom = useAITeacher((state) => state.classroom);
   ...
   return (
      ...
      >
         {messages.length === 0 && (
            <div className="h-full w-full grid place-content-center text-center">
               <h2 className="text-8xl font-bold text-white/90 italic">
                  Manav Rachna University Assistant
               </h2>
            </div>
         )}
         {messages.map((message, i) => (
            <div key={i}>
               ...
               <div className="px-5 py-2 mt-2 bg-gradient-to-br from-pink-200/20 to-pink-500/20 rounded-lg">
                  <span className="pr-2 italic bg-clip-text text-transparent bg-gradient-to-b from-white/90 to-white/70 text-3xl font-semibold inline-block">
                     Made with ❤️ at Ibentos
                  </span>
               </div>
            </div>
         ))}
